	...
	public static void main(String[] args) throws Exception {
		...
		while (!exitProgram) {
			...
			if (role.equals("admin")) {
				...
				signedIn = studentOperations.studentLogin(ID);				if (!signedIn) {
				System.out.println("!!!!!!!!!!! User has already signed in with other system.");
				} else {
					System.out.println("@@@ " + ID + " Connected to server");
					logger.log("\"@@@ \"+ID+\" Connected to server\"");
			...
			while (signedIn) {
				System.out.println("\n\nWelcome " + role + " " + ID
						+ " to the menu. Please choose an operation: (TEST CONCURRENT)");
				try {
					if (role.equals("admin")) {
						System.out.println("1- (TEST CONCURRENT) Create room");
						System.out.println("2- (TEST CONCURRENT) Delete room");
						System.out.println("3- (TEST CONCURRENT) ** KILL SERVER **");
						System.out.println("4- (TEST CONCURRENT) ** FAKE GENERATOR **");
						...
						case (1):
							...
							while (true) {
								AdminOperations tempAdminOperation = adminOperations==null?null:adminOperations;
								if (LocalTime.now().isAfter(startTaskCreate)) {
									Runnable runnable = () -> {
										try {
											String createResult = tempAdminOperation.createRoom(idCre, roomNumberCre,
													dateCre, timesCre);
							...
							while (true) {
								...
								if (LocalTime.now().isAfter(startTaskDelete)) {
									Runnable runnable = () -> {
										...
										try {
											String deleteResult = tempAdminOperation.deleteRoom(idDel, roomNumberDelete,
													dateDelete, timesDelete);
						...
						System.out.println("5- (TEST CONCURRENT) ** KILL SERVER **");
						System.out.println("6- (TEST CONCURRENT) ** FAKE GENERATOR **");
						...
						case (1):
							...
							String startBookTime = InputManager.getTime("Start and End");

							System.out.println("==== W A I T  T O  S T A R T  T H E  T A S K ===");

							while (true) {
								StudentOperations tempStudentOperations = studentOperations==null?null:studentOperations;

								if (LocalTime.now().isAfter(startTaskBook)) {

									Runnable runnable = () -> {
										System.out.println("bookroom: "+ Thread.currentThread().getId() + " time:" + startBookTime);
										String bookResult;
										try {
											bookResult = tempStudentOperations.bookRoom(idBook, 
													campusBook == 1 ? "DVL" : campusBook == 2 ? "KKL" : "WST", roomBook,
													dateBook, startBookTime);
							...
							while (true) {
								...
								if (LocalTime.now().isAfter(startTaskGetTime)) {
									...
									Runnable runnable = () -> {
										...
										try {
											getTimeResult = tempStudentOperations.getAvailableTimeSlot(idGetTime,
													dateGetTime);
							...
							while (true) {
								...
								if (LocalTime.now().isAfter(startTaskCancel)) {
									Runnable runnable = () -> {
										String cancelResult;
										try {
											cancelResult = tempStudentOperations.cancelBooking(idCancel, bookingIdCancel);
							...
							while (true) {
								...
								if (LocalTime.now().isAfter(startTaskChange)) {
									Runnable runnable = () -> {
										...
										try {
											changeTimeSlotResult = tempStudentOperations.changeReservation(idChange,
													bookingIdChange, 
													campusChangeNum == 1 ? "DVL" : campusChangeNum == 2 ? "KKL" : "WST", roomChange, startTimeChange,
													endTimeChange);
