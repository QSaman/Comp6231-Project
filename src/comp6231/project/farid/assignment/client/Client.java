...
// Client can connect to 3 servers such as Dorval, Kirkland and Westmount. User role could be either Student or Admin
public class Client {
			...
			while (signedIn) {
				...
				try {
					...
					if (role.equals("admin")) {
						...
						switch (choice) {
						case (1):
							String createResult = serverInterface.createRoom(ID, InputManager.getNumber("room Number"),
									InputManager.getDate(), InputManager.getListOfTimesFromAdmin());
							System.out.println(createResult);
							logger.log(createResult);
							break;
						case (2):
							String deleteResult = serverInterface.deleteRoom(ID, InputManager.getNumber("room Number"),
									InputManager.getDate(), InputManager.getListOfTimesFromAdmin());
						...
						case (1):
							...
							String bookResult = serverInterface.bookRoom(ID, campus,
									InputManager.getNumber("Room Number"), InputManager.getDate(),
									InputManager.getTime("Start"), InputManager.getTime("End"));
							System.out.println(bookResult);
							logger.log(bookResult);
							break;
						case (2):
							String getTimeResult = serverInterface.getAvailableTimeSlot(ID, InputManager.getDate());
							System.out.println(getTimeResult);
							logger.log(getTimeResult);
							break;
						case (3):
							String cancelResult = serverInterface.cancelBooking(ID,
						...
						case (4):
							...
							String changeTimeSlotResult = serverInterface.changeReservation(ID,
									InputManager.getString("booking ID"), campusChange,
									InputManager.getNumber("room number"), InputManager.getTime("start"),
									InputManager.getTime("end"));
