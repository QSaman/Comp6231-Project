...
public class Student {
	...
	private boolean isStudentBookingCounterWithAllServersOK() throws Exception {
		...
		DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 9763);
		...
		DatagramPacket sendPacket2 = new DatagramPacket(sendData2, sendData2.length, IPAddress2, 9635);
	...
	String getAvailableTimeSlot(LocalDate date) throws Exception {
		...
		Thread threadWST = new Thread(() -> {
			try {
				wstNumber = Integer.parseInt(getNumberOfAvailableTimesFromOtherServer(9635, date).trim());
			} catch (Exception e) {
				// e.printStackTrace();
			}
		});
		...
		if (bookingID.startsWith("KKL")) {
			...
			try {
				synchronized (Locker.reserveLock) {
					if (assignment.serverKirkland.ReserveManager.reserveMap.containsKey(bookingID)
							&& assignment.serverKirkland.ReserveManager.reserveMap.get(bookingID).getStudentID()
									.equals(studentID)) {
						assignment.serverKirkland.ReserveManager.counterDB.get(studentID).decrementCounter();
						String bookResult = bookRoom(campus, roomNumber,
								assignment.serverKirkland.ReserveManager.reserveMap.get(bookingID).getDate(), startTime,
								endTime);
						Scanner scanner = new Scanner(bookResult);
						scanner.nextLine();
						scanner.nextLine();
						String tempResultOfBook = scanner.nextLine();
						assignment.serverKirkland.ReserveManager.counterDB.get(studentID).incrementCounter();
						if (tempResultOfBook.substring(tempResultOfBook.indexOf("/")).charAt(8) == 's') {
							resultToSendToStudent.append(
									assignment.serverKirkland.ReserveManager.removeFromReserveList(studentID, bookingID)
											? "\nThe booking number " + bookingID + " canceled successfully."
											: "\n The booking number " + bookingID + " failed canceling.");
			...
			assignment.serverKirkland.ReserveManager.printCurrentReserveList();
			ServerKirkland.printCurrentDatabase();
			return resultToSendToStudent.toString();
		} else {
			int port = 0;
