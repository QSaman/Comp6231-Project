...
public class Student implements Serializable {
			...
			if (!ReserveManager.counterDB.containsKey(studentID)
				...
				try {
					synchronized (Locker.databaseLock) {
						if (ServerDorval.dataBase.get(date).get(roomNumber).containsKey(startTime)
								&& ServerDorval.dataBase.get(date).get(roomNumber).get(startTime).equals(endTime)) {

							ServerDorval.dataBase.get(date).get(roomNumber).remove(startTime);
	...
	private boolean isStudentBookingCounterWithAllServersOK() throws Exception {
		...
		DatagramPacket sendPacket2 = new DatagramPacket(sendData2, sendData2.length, IPAddress2, Constants.WST_PORT_LISTEN_FARID_ACTIVE);
		...
		int total;
		synchronized (Locker.counterLock) {
			int counterOnDVL = (ReserveManager.counterDB.containsKey(studentID)
		...
		if (isStudentBookingCounterWithAllServersOK()) {
			if (campus != 1) {
				...
				if (campus == 2)
					...
					port = Constants.WST_PORT_LISTEN_FARID_ACTIVE;
	...
	String getAvailableTimeSlot(LocalDate date) throws Exception {
		...
		Thread threadWST = new Thread(() -> {
			try {
				wstNumber = Integer.parseInt(getNumberOfAvailableTimesFromOtherServer(Constants.WST_PORT_LISTEN_FARID_ACTIVE, date).trim());
			} catch (Exception e) {
				//e.printStackTrace();
			}
		});
		...
		Thread threadDVL = new Thread(() -> {
			try {
				dvlNumber = getAvailableTimeFromServerForDate(date);
			} catch (Exception e) {
				// e.printStackTrace();
			}
		});
	...
	static int getAvailableTimeFromServerForDate(LocalDate date) {
		...
		synchronized (Locker.databaseLock) {
			try {
				for (HashMap.Entry<Integer, HashMap<LocalTime, LocalTime>> entry : ServerDorval.dataBase.get(date)
						.entrySet()) {
	...
	private String getNewBookingID() {
		...
		result = "DVL-" + stringBuilder.toString();
		synchronized (Locker.reserveLock) {
