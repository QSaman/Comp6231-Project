		...
		try {
			serverSocket = new ReliableServerSocket(
					udp_name.equals("wst_org") ? Constants.WST_PORT_LISTEN_FARID_ORIGINAL
							: Constants.WST_PORT_LISTEN_FARID_BACKUP, 0 , InetAddress.getByName(Constants.FARID_IP));
			while (true) {
		...
		private String processData(String json) throws Exception {
			...
			if (json_msg.protocol_type == ProtocolType.Frontend_To_Replica) {
				...
				if(json_msg.command_type == CommandType.Kill) {
					...
					ServerWestmount.setFakeGeneratorOff(message.turnOff);
					ServerWestmount.westmountServerLogger.log(" isFakeGeneratorOn : " + !ServerWestmount.isFakeGeneratorOff());
				}else {
					ServerWestmount.westmountServerLogger.log(" command type is wrong for rm request");
				}
			}else {
